...
## Database migrations (v0.0.9)

Schema changes go through Alembic, not `create_all` (which can only add
missing tables, never columns). Startup applies pending migrations
...
Review every autogenerated file before committing — two known gaps are
already encoded in `alembic/versions/` as examples: `server_default` must
be added by hand for NOT NULL columns joining populated tables, and
SQLite needs batch mode (already on in `env.py`).

Pre-0.0.9 databases have tables but no Alembic stamp: delete `app.db`,
or `alembic stamp 0001` once and upgrade from there.

## Admin API (v0.0.9)

Course/lesson/vocabulary/quiz content is managed through `/admin/*`
(see `app/routers/admin.py`). Promote a user from the backend directory:

```bash
python scripts/make_admin.py <username>            # grant
python scripts/make_admin.py <username> --revoke   # revoke
```
